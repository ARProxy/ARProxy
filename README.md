...
- AWS EC2(CPU/GPU) 기반 PoC 환경에서 4개의 모듈을 블랙박스로 정의하고 어댑터 패턴으로 역할 캡슐화
- AOP 기반 모듈 사용 이력 및 파일 처리 로깅 시스템 설계
- 외국인등록증 OCR 정확도 onnx모델 서빙에 대한 전처리, 후처리 개선으로 인한 88% → 96%로 향상
- 멀티 모듈 환경에서의 운영 가시성 확보를 목표로, 로그 포맷을 표준화하고 Loki 기반 중앙 로그 수집
- PoC 대응 속도 개선을 위한 메트릭 기반 알림 서비스 구축
